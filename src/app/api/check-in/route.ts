...
export async function POST(req: NextRequest) {
  try {
    ...
    // Unique email per Laravel validation
    ...
    // Collect all values per key (supports checkbox groups with same name)
    ...
    for (const [k, values] of collected.entries()) {
      if (values.length > 1) {
        // Checkbox group: normalize each entry and keep as array
